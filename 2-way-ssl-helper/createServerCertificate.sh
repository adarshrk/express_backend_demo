...
# Note: Run the shell script using sudo command only, else permission denied
...
echo '$localhost_ip  $server_hostname' >> /etc/hosts
...
echo 'creating server certificate folder'
rm -rf serverCerts
mkdir serverCerts

openssl req -new -x509 -days 365 -keyout serverCerts/server-ca-key.pem -out serverCerts/server-ca-crt.pem
...
echo 'Now we generate the actual server certificate which will be used in the ssl handshake.'
# First of all we have to generate a random key (4096 bit length in our example)
openssl genrsa -out serverCerts/server-key.pem 4096

echo 'Next we generate a Certificate Signing Request (CSR) with the key we have generated'
openssl req -new -sha256 -key serverCerts/server-key.pem -out serverCerts/server-csr.pem
...
echo 'As final step, generate the server certificate (validity 1 year) from the CSR previously created and sign it with the CA key'
openssl x509 -req -days 365 -in serverCerts/server-csr.pem -CA serverCerts/server-ca-crt.pem -CAkey serverCerts/server-ca-key.pem -CAcreateserial -out serverCerts/server-crt.pem
...
openssl verify -CAfile serverCerts/server-ca-crt.pem serverCerts/server-crt.pem
